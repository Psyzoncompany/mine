...
const keyMap={KeyA:'left',ArrowLeft:'left',KeyD:'right',ArrowRight:'right',Space:'jump',KeyW:'up',ArrowUp:'up',KeyS:'down',ArrowDown:'down',KeyR:'technique',KeyJ:'attack',KeyK:'blast',KeyL:'special',ShiftLeft:'dash',ShiftRight:'dash',KeyI:'guard',KeyU:'charge',KeyT:'transform',KeyV:'vanish'};
...
let touchLayout;
let campaign=readProgress(),saga=readSaga(),campaignMode='story',storySaga=sagaRecord(readSaga(),103).completed?'freeza':'saiyan',selectedStage=101,selectedSkill='vigor',skillsWasPlaying=false,lastStoryReward=0,progressSaved=true;
const storyStages=()=>storySaga==='freeza'?FREEZA_CHAPTERS:SAIYAN_CHAPTERS;
const mapStages=()=>campaignMode==='story'?storyStages():STAGES;
const getStage=id=>[...CHAPTERS,...STAGES].find(s=>s.id===id);
const unlocked=id=>id>=100?sagaUnlocked(saga,id):isUnlocked(campaign,id);
const recordFor=id=>id>=100?sagaRecord(saga,id):stageRecord(campaign,id);
...
function begin(stageId=engine.stageId){
  ...
  if(!loaded)return;if(touch&&!document.body.classList.contains('immersive'))enterFullscreen();sound.unlock();clearInput();engine.start(stageId,saga);particlePool.clear();particles=particlePool.items;effects=[];trails=[];cam=0;cameraZoom=1;zoomImpulse=0;
  ...
  document.body.dataset.level=String(stageId);document.body.dataset.saga=engine.story?.saga||'saiyan';canvas.setAttribute('aria-label',engine.level.name+', jogo de plataforma e combate');document.body.classList.remove('transformed');stage.focus({preventScroll:true});hudUpdate();
  drawPortrait();
}
...
function clearInput(){controller.suppress();keys.clear();pointers.clear();engine.inputBuffer={};engine.p.queued=null;Object.keys(pressed).forEach(k=>delete pressed[k]);document.querySelectorAll('[data-action]').forEach(b=>b.classList.remove('held'));}
function getInput(){const gamepad=pollController();const data={...gamepad,pressed:{...gamepad.pressed,...pressed}};for(const k of keys)if(keyMap[k])data[keyMap[k]]=true;for(const a of pointers.values())data[a]=true;Object.keys(pressed).forEach(k=>delete pressed[k]);return data;}
window.addEventListener('keydown',e=>{
  ...
  if(engine.dialogue){if(['Enter','Space','KeyJ','Escape'].includes(e.code)){e.preventDefault();if(!e.repeat){engine.advanceDialogue(e.code==='Escape');clearInput();}}return;}
  ...
  const action=keyMap[e.code];if(action&&engine.mode==='playing'){if(!keys.has(e.code))pressed[action]=true;keys.add(e.code);}
});
stage.addEventListener('pointerdown',()=>{document.body.classList.remove('using-controller');syncScreen();});
window.addEventListener('keyup',e=>{if(keyMap[e.code]==='jump'&&keys.has(e.code))pressed.jumpRelease=true;keys.delete(e.code);});
...
function hudUpdate(){
  ...
  $('timing-cue').textContent=engine.saiyanCombat&&!engine.versus?.finish?(p.stun>0?(p.escapeCooldown>0?'RECUPERAÇÃO EM '+p.escapeCooldown.toFixed(1)+'s':'DEFESA / ESQUIVA · RECUPERAR 25 KI'):a?.connected&&a.step>=1?'CHUTE · AFASTAR  |  ELEVAR · 14 KI  |  KAME · '+(engine.skills.has('kame')?32:40)+' KI':''):'';
...
function processEvents(){
  ...
  for(const e of events){
    ...
    switch(e.type){
      ...
      case 'toast':showToast(controllerConnected?(e.text.replaceAll('A / D','ANALÓGICO').replaceAll('ESPAÇO','A').replaceAll('J:','X:').replaceAll('R:','Y:').replaceAll('Aperte T','Pressione L3').replaceAll('aperte T','pressione L3').replaceAll('Segure U','Segure LT').replaceAll('segure U','segure LT')):touch&&e.mobile?e.mobile:e.text);break;
...
function buildAtlas(image,cols,rows){
  ...
  const sagaAsset=/goku-v8|kaioken-v8|saga-enemies-v8/.test(image.src||'');
  ...
  if(/freeza-v20|saiyan-bosses-v21/.test(image.src||'')){
    ...
    const completeFrames=parts.map((f,i)=>{
      ...
      return {rect:[tx,ty,w,h],anchor:f.anchor};
    });
...
function renderEnemyBody(e){
  ...
  if(engine.versus){const atlas=fighterAtlas(engine.versus.opponent),f=atlas.frames[versusPose(engine.versus.opponent,e)];shadow(x,e.grounded?e.y:WORLD.ground,e.w*.55,.25);sprite(atlas.image,f.rect,x,e.y,e.dir,f.scale??atlas.scale,{anchor:f.anchor,hurt:e.flash>.04});drawBossStyle(e,x);drawVersusTag(x,e.y-engine.versus.opponent.height-12,'CPU','#ffabb4');return;}
  ...
  if(e.storyBoss==='freeza'){const atlas=atlases.freeza,f=atlas.frames[freezaPose(e)];sprite(atlas.image,f.rect,x,y,e.dir,atlas.scale,{anchor:f.anchor,hurt:e.flash>.04,alpha:e.hp<=0?e.dead/.65:1});}
  else if(engine.story&&(e.type===2||engine.story.chapter>1)){const atlas=engine.saiyanCombat&&boss&&e.spriteRow<3?atlases.saiyanBosses:engine.story.saga==='freeza'?atlases.namekVillains:atlases.sagaEnemies,index=e.hp<=0?5:e.stun>0?4:e.state==='windup'?2:e.state==='attack'?3:['walk','run','flight','evade','retreat','flank'].includes(e.state)?1:0;const f=atlas.frames[e.spriteRow*6+index],h=atlas.frames[e.spriteRow*6].rect[3];sprite(atlas.image,f.rect,x,y,e.dir,(engine.story.saga!=='freeza'&&e.spriteRow===3?205:boss?125:84)/h,{anchor:f.anchor,hurt:e.flash>.04,alpha:e.hp<=0?e.dead/.65:1});}
  else if(e.saiyan){const atlas=atlases.saiyans,index=e.hp<=0?5:e.stun>0?4:e.state==='windup'?2:e.state==='attack'?3:['walk','run','flight','evade','retreat','flank'].includes(e.state)?1:0;const f=atlas.frames[e.type*6+index];const h=atlas.frames[e.type*6].rect[3];sprite(atlas.image,f.rect,x,y,e.dir,(boss?135:105)/h,{anchor:f.anchor,hurt:e.flash>.04,alpha:e.hp<=0?e.dead/.65:1});}
  else if(e.hp>0&&defenseFrame>=0){const atlas=atlases.defense,f=atlas.frames[e.type*6+defenseFrame],rowHeight=Math.max(...atlas.frames.slice(e.type*6,e.type*6+6).map(f=>f.rect[3]));sprite(atlas.image,f.rect,x,y,e.dir,(boss?150:106)/rowHeight,{anchor:f.anchor,hurt:e.flash>.04});}
  else {ctx.save();ctx.translate(x,y);const anticipation=e.state==='windup'?Math.sin(clamp(1-e.timer/(e.windupDuration||.6),0,1)*Math.PI/2):0;ctx.transform(1,0,e.dir*anticipation*.065,1,0,0);ctx.scale(1+anticipation*.045,1-anticipation*.045);sprite(images.enemies,enemyRects[e.type*6+frame],0,0,e.dir,scale+breathe,{hurt:e.flash>.04,alpha:e.hp<=0?e.dead/.65:1});ctx.restore();}
...
function renderPlayer(){
  ...
  if(p.state==='dash'||p.attack||p.flying||p.form&&Math.abs(p.vx)>150){if(trails.length===0||engine.visualTime-trails[trails.length-1].t>.045)trails.push({x:p.x,y:p.y,frame,combo:pose.combo,dir:p.dir,solar:p.form,t:engine.visualTime,life:.2});}
  for(const t of trails){const a=t.combo?atlases.saiyanCombos:playerAtlas(t.solar),f=a.frames[t.frame];sprite(a.image,f.rect,t.x-cam,t.y+(a.offsetY||0)*a.scale,t.dir,f.scale??a.scale,{alpha:Math.max(0,(engine.saiyanCombat?.12:.2)*(t.life/.2)),anchor:f.anchor});}
...
function render(dt){
  ...
  const adaptive=opponent&&engine.mode!=='intro';let target=clamp(p.x-W*.35,0,Math.max(0,WORLD.width-W));
...
function renderMap(){
  ...
  nodes.replaceChildren();for(const level of levels){
    ...
    if(level.chapter){art.width=200;art.height=150;const ac=art.getContext('2d');ac.imageSmoothingEnabled=false;const atlas=freeza?atlases.namekVillains:atlases.sagaEnemies,row=freeza?({201:1,202:2,203:4,204:5}[level.id]):level.chapter-1;const [sx,sy,sw,sh]=atlas.frames[row*6].rect,scale=Math.min(170/sw,140/sh);ac.drawImage(atlas.image,sx,sy,sw,sh,(200-sw*scale)/2,150-sh*scale,sw*scale,sh*scale);}else art.style.backgroundPosition=level.crop.join('% ')+'%';
    const number=document.createElement('small');number.textContent=level.chapter?'CAPÍTULO '+level.chapter:'FASE '+String(level.id).padStart(2,'0');
    const name=document.createElement('strong');name.textContent=level.chapter?level.bossName:level.name;
  ...
  const level=getStage(selectedStage),record=recordFor(selectedStage),ready=unlocked(selectedStage);$('map-number').textContent=level.biome;$('map-title').textContent=level.name;$('map-description').textContent=level.detail;$('map-enemies').textContent=level.enemy;$('map-goal').textContent=level.available?'7 esferas + '+(level.bossName||'chefe'):'PRÓXIMA SAGA';
  $('map-preview').style.backgroundImage=level.saga==='freeza'?"url('/assets/namek-stage-v13.png')":level.id===2?"url('/assets/forest-v6.png')":level.chapter||level.id===1?"url('/assets/valley.png')":"url('/assets/world-map-v5.png')";$('map-preview').style.backgroundSize=level.chapter||level.id<=2?'cover':'280%';
  $('map-play').disabled=!level.available||!ready;$('map-play-label').textContent=!level.available?'EM DESENVOLVIMENTO':!ready?'CONCLUA O CAPÍTULO ANTERIOR':record.completed?'JOGAR NOVAMENTE':'INICIAR '+(level.chapter?'CAPÍTULO':'FASE');
  ...
  const records=campaignMode==='story'?levels.map(l=>sagaRecord(saga,l.id)):Object.values(campaign.stages),completed=records.filter(r=>r.completed).length;$('map-orbs').textContent=records.reduce((n,r)=>n+r.orbs,0)+' / '+total*7;$('map-completed').textContent=completed+' / '+total;$('map-best-combo').textContent=Math.max(0,...records.map(r=>r.bestCombo));$('map-save-note').textContent=progressSaved?'Progresso salvo neste dispositivo.':'Progresso disponível apenas nesta sessão.';
}

...
export class GameEngine {
  ...
  reset(stageId=1,build={}){
    ...
    this.encounters=(this.story?storyEncounters(this.story):this.stageId===2?FOREST.encounters:ENCOUNTERS).map(e=>({...e,active:false,cleared:false}));this.activeEncounter=null;
  ...
  start(stageId=1,build={}){this.reset(stageId,build);this.mode='playing';if(this.story){this.enemies.at(-1).name=this.story.bossName;this.encounters.at(-1).title=this.story.bossName;this.openScene('intro');}this.emit('zone',{title:this.level.name,subtitle:this.story?'Reúna as 7 esferas e alcance '+this.story.bossName+'.':this.stageId===2?'Reúna 7 esferas. Vença a patrulha e o comandante Razek.':'Encontre as 7 esferas. Derrote o guardião.'});this.emit('toast',{text:'A / D: mover · ESPAÇO: pular · J: golpes · R: elevar / queda',mobile:'GOLPE para combinar · ELEVAR lança o alvo · QUEDA finaliza no ar.'});}
  get collected(){return this.orbs.filter(o=>o.got).length;}
  get saiyanCombat(){return isSaiyanChapter(this.stageId);}
  ...
  get progress(){return clamp((this.p.x-150)/(WORLD.width-400),0,1);}
  ...
  updatePlayer(dt,input){
    ...
    const technique=pressed.launcher||pressed.technique&&p.grounded||pressed.attack&&input.up;
    const slam=pressed.slam||pressed.technique&&!p.grounded||pressed.attack&&input.down&&!p.grounded;
    const spin=this.saiyanCombat&&pressed.blast&&!input.charge&&p.attack&&p.attack.step>=1;
    if(spin){input={...input,blast:false};pressed.blast=false;}
    if(p.attack&&(pressed.attack||technique||slam||spin)){
      const a=p.attack,perfect=a.t>=a.move.end-.045&&a.t<=a.move.end+.095;
      p.queued={kind:slam?'slam':technique?'launch':spin?'spin':'normal',perfect};
    }
    ...
    if(pressed.attack&&p.dashTimer>0){p.dashTimer=0;p.dragonRush=false;p.recovery=0;}

    if(this.saiyanCombat&&input.guard&&(pressed.vanish||pressed.dash&&p.stun<=0)){this.defensiveVanish();pressed.vanish=false;pressed.dash=false;}
    ...
    if(p.stun>0){p.state='hurt';p.vx*=Math.pow(.12,dt);}
      ...
      if(pressed.transform&&p.form&&this.story){p.form=false;p.formTime=0;p.formCooldown=3;this.emit('formEnd');}
        ...
        else this.tip(this.story?'Kaioken precisa de 60 KI.':'Transformação precisa de 100 KI. Acerte ou segure U para carregar.');
      ...
      // Aerial pursuit consumes the dash edge and keeps the new attack active.
	        ...
	        p.stamina-=16;
        p.state='dash';p.stateTime=0;p.dashTimer=.19;p.dashCooldown=this.story&&this.skills.has('guard')?.48:.65;p.invincible=.26;p.attack=null;p.queued=null;p.diving=false;
        if(move)p.dir=sign(move);p.vx=p.dir*(p.form?900:790);p.vy*=.25;this.emit('dash',{x:p.x,y:p.y-35,dir:p.dir});
      } else if(p.dragonRush){p.state='dash';p.dashTimer=.08;} else if(p.dashTimer>0){p.dashTimer-=dt;p.state='dash';p.vx=p.dir*(p.form?900:790);p.vy*=.8;}
      else{
	        if(input.guard&&p.guardMeter>0&&(this.saiyanCombat||!p.attack||p.attack.connected||p.attack.t>p.attack.move.end)){p.attack=null;p.queued=null;p.guarding=true;p.guardTime+=dt;p.state='guard';p.vx=move*60;if(move)p.dir=sign(move);}
          ...
          if((pressed.blast||input.blast)&&p.blastCooldown<=0&&!p.attack){
            ...
            else if(pressed.blast)this.tip('Sem KI. Acerte golpes ou segure U para carregar.');
          }
      ...
      if(a.t>=m.active&&a.t<=m.end){
        for(const e of this.enemies){
          if(e.hp<=0||a.hit.has(e.id))continue;
            ...
            a.hit.add(e.id);const hit=this.hitEnemy(e,m.damage*(this.story&&this.skills.has('combo')?1.15:1)*(p.form?(this.story?1.25+this.kaiokenLevel*.18:1.4):1)*(a.perfect?1.25:1),m.knock,p.dir,{finisher:this.saiyanCombat?!!m.finisher:p.grounded&&a.step>=2,launch:a.kind==='launch'||!this.saiyanCombat&&p.form&&a.step===3,slam:a.kind==='slam',aerial:!p.grounded,perfect:a.perfect,hitstunFrames:this.saiyanCombat?m.hitstunFrames:undefined});
  ...
  beginAttack(kind='normal',perfect=false){
    const p=this.p;if(kind==='slam'&&p.grounded)kind='launch';
  ...
  hitEnemy(e,damage,knock,dir,options={}){
    ...
    e.hp=Math.max(0,e.hp-damage);e.flash=.15;
    ...
    this.freeze=Math.max(this.freeze,options.beam?.065:options.finisher?.055:options.perfect?.035:.02);
  ...
  hitPlayer(damage,sourceX,knock=220,unblockable=false,options={}){
    ...
    if(p.guarding&&!unblockable&&(sourceX-p.x)*p.dir> -15){
      if(p.guardTime<(this.saiyanCombat?(this.skills.has('guard')?.12:.085):(this.story&&this.skills.has('guard')?.29:.2))&&(!this.saiyanCombat||p.parryCooldown<=0)){
        if(this.saiyanCombat)p.parryCooldown=.45;
        this.parries++;p.ki=clamp(p.ki+16,0,100);p.invincible=.4;this.freeze=.09;
        this.emit('parry',{x:p.x+p.dir*30,y:p.y-45});this.tip('APARO PERFEITO · +16 KI');
        const attacker=this.enemies.find(e=>e.hp>0&&Math.abs(e.x-sourceX)<10);if(attacker){attacker.stun=.85;attacker.state='hurt';attacker.timer=.85;}
    ...
    this.freeze=.035;this.emit('playerHit',{x:p.x,y:p.y-45,dir});
  ...
  enemyMelee(e,boss){
    ...
    const options={hitstunFrames:kind==='counter'?24:kind==='launcher'?22:kind==='finisher'?25:15,launch:kind==='launcher',slam:kind==='finisher',heavy:['counter','launcher','finisher'].includes(kind),combo:true};
  ...
  updateEnemy(e,dt){
    e.moveCycle+=dt;e.flash=Math.max(0,e.flash-dt);e.invincible=Math.max(0,e.invincible-dt);e.cooldown-=dt;e.timer-=dt;e.defenseCooldown-=dt;e.launchTime=Math.max(0,e.launchTime-dt);e.airFloat=Math.max(0,e.airFloat-dt);
  ...
  openScene(key){
    const lines=(this.saiyanCombat?SAIYAN_BOSS_SCENES:SCENES)[this.stageId]?.[key];
  ...
  advanceDialogue(skip=false){
    ...
    this.mode=d.after;this.dialogue=null;this.inputBuffer={};this.p.invincible=Math.max(this.p.invincible,.5);this.emit('dialogueEnd');if(this.mode==='won')this.emit('victory');
  }
  updateStoryBoss(e){
    if(e.hp<=0)return false;
